...
function renderAiOrganization() {
  ...
  allNodes.forEach(node => {
    node.addEventListener("click", () => {
      ...
      // 統合された個別相談パネルを表示して、そこへスクロール
      const consultPanel = $("consultation-panel");
      if (consultPanel) {
        consultPanel.style.display = "block";
        consultPanel.scrollIntoView({ behavior: "smooth", block: "nearest" });
      }
      
      // スクロール後に自動フォーカス
      setTimeout(() => {
        const promptInput = $("user-prompt");
...
function selectDepartment(deptId) {
  ...
  // プロンプト入力欄をクリアし、プレースホルダーに例を設定
  $("user-prompt").value = "";
  
  const placeholders = {
    ceo: "例：那須ミッドシティホテルの今期の経営方針について、ROIを重視した判断基準を教えてください。",
    coo: "例：今週の役員会議に向けて、公式予約率向上のための90日アクションプランを統合・整理してください。",
    "ai-trends": "例：GeminiやGoogle AI Modeが旅行者の検索に与える影響について、最適なテキスト表現を相談します。",
    "us-insights": "例：米国ホテルで導入されている、客室QRコードを起点としたアップセルの最新事例を教えてください。",
    "marketing-ota": "例：じゃらんや楽天の手数料を削減し、公式HPへの予約率を高めるためのキャンペーン施策を提案してください。",
    "ai-agent-booking": "例：将来、旅行者がAIエージェントに予約を任せる時代に優先推薦されるためのSEO・構造化データ設計を相談します。",
    "revenue-management": "例：稼働率と競合価格に連動した、ビジネス客および観光ファミリー向けのダイナミックプライシング枠組みを策定します。",
    "restaurant-strategy": "例：朝食バイキング（那須御養卵TKG等）の価値向上と、夕食提携店での売上アップ施策を相談します。",
    "digital-guide": "例：客室QRの案内サイトをデジタルコンシェルジュへと進化させるための具体的な遷移設計を作成してください。",
    "hotel-ops": "例：深夜帯のフロント業務自動化や、AIチャットボット導入時のスタッフ向け教育手順について相談します。"
  };
  
  $("user-prompt").placeholder = placeholders[deptId] || "相談内容を入力してください...";
  ...
  // 部署に応じた動的おすすめプロンプトチップスの表示
  const recommendPrompts = target.recommendPrompts || window.HOTEL_KNOWLEDGE.promptChips;
  $("prompt-chips").innerHTML = recommendPrompts.map((prompt) => `
    <button class="prompt-chip" data-prompt="${prompt}">${prompt}</button>
  `).join("");

  document.querySelectorAll("#prompt-chips .prompt-chip").forEach((button) => {
  ...
  // 詳細カード内のおすすめ質問ボタンの動的描画
  const questionsSection = $("org-detail-questions-section");
  const questionsGrid = $("org-detail-questions-grid");
  
  if (questionsSection && questionsGrid) {
    if (recommendPrompts && recommendPrompts.length > 0) {
      questionsGrid.innerHTML = recommendPrompts.map(prompt => `
        <button class="org-detail-question-btn" data-prompt="${prompt}">
      ...
      // クリックイベントの登録
      questionsGrid.querySelectorAll(".org-detail-question-btn").forEach(btn => {
        btn.addEventListener("click", () => {
          const promptText = btn.dataset.prompt;
          triggerDirectConsult(deptId, promptText);
        });
      });
    } else {
...
function renderQrGuides() {
  ...
  container.querySelectorAll(".qr-consult-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // 組織図セクションへ切り替え
      const sideNavOrgLink = document.querySelector(".side-nav a[href='#org-chart']");
      if (sideNavOrgLink) {
        sideNavOrgLink.click();
      }
...
function renderPromptChips() {
  // selectDepartmentが動的描画を行うため、ここでは初期表示のみ実行
  if (state.selectedDept) {
    selectDepartment(state.selectedDept);
  }
}
...
// 6. サイドナビによる表示切り替え制御（会議以外は非表示にし、左の項目クリックで切り替え）
function initSideNavTabs() {
  const links = document.querySelectorAll(".side-nav a:not(#side-nav-db)");
  const sections = [
    $("overview"),
    $("site-audit-section"),
    $("org-chart"),
  ...
  // 初期表示：AI役員会・組織図(org-chart)のみを表示
  ...
  // アクティブなサイドバーリンクの初期設定（初期表示は組織図）
  links.forEach(l => {
    if (l.getAttribute("href") === "#org-chart") {
  ...
  links.forEach(link => {
    link.addEventListener("click", (e) => {
      ...
      const targetId = hrefVal.substring(1);
      ...
      // 表示の制御
      if (targetId === "assistant") {
        // 会議の場合は、会議室、会議メモ、相談履歴を表示する
        $("assistant").style.display = "block";
        if ($("notes")) $("notes").style.display = "block";
        if ($("ai-history")) $("ai-history").style.display = "block";
      } else {
        targetSec.style.display = "block";

        // 現状サイトの観測は、全体戦略（overview）と一緒に表示する
        if (targetId === "overview") {
          const auditSec = $("site-audit-section");
          if (auditSec) auditSec.style.display = "grid";
        }
      }
...
const themeDeptsMap = {
  ...
  "webmcp-booking": ["ai-agent-booking", "hotel-ops", "marketing-ota", "ai-trends", "us-insights"],
  "qr-migration": ["digital-guide", "marketing-ota", "ai-trends", "hotel-ops"],
  "new-pages-seo": ["ai-agent-booking", "restaurant-strategy", "marketing-ota", "digital-guide"],
...
async function askAssistant() {
  ...
  try {
    ...
    // 状態を保存
    localStorage.setItem("nasu_consult_answer", data.answer || "");
    localStorage.setItem("nasu_consult_status_class", "status-pill success");
    localStorage.setItem("nasu_consult_status_text", "回答完了");
    localStorage.setItem("nasu_consult_selected_dept", state.selectedDept);
...
${customAgenda ? `【特記指示事項】：\n${customAgenda}` : ""}`;
  ...
  try {
    ...
    // 状態を保存
    localStorage.setItem("nasu_boardroom_flow_html", $("discussion-flow").innerHTML);
    localStorage.setItem("nasu_boardroom_report", finalReportText);
    localStorage.setItem("nasu_boardroom_status_class", "status-pill success");
    localStorage.setItem("nasu_boardroom_status_text", "会議終了・報告書完成");
    localStorage.setItem("nasu_boardroom_theme", themeValue);
    localStorage.setItem("nasu_boardroom_custom_agenda", customAgenda);
...
function initEvents() {
  ...
  // 共通データ基盤モーダル開閉
  const dbLink = $("side-nav-db");
  const dbModal = $("database-modal");
  const dbCloseBtn = $("database-modal-close");
  ...
  // アコーディオンのトグル制御
  const accordion = $("context-accordion");
  const accHeader = $("context-accordion-header");
  const accTitle = accHeader.querySelector(".accordion-title-text");

  // 初期読み込み時にテキストエリアに値があれば開いておく
  if ($("meeting-context").value.trim() !== "") {
    accordion.classList.add("active");
    accTitle.innerHTML = `<i class="fas fa-file-invoice"></i> 前提データ・会議補足メモ (入力中/展開中)`;
  }
  ...
  accHeader.addEventListener("click", () => {
    ...
    if (accordion.classList.contains("active")) {
      ...
      accTitle.innerHTML = `<i class="fas fa-file-invoice"></i> 前提データ・会議補足メモを追加する (任意・折りたたみ中)`;
    }
  });
  ...
  // ローカルブラウザでのフォールバック音声読み上げ用
  function speakTextLocal(text, btnEl) {
    ...
    utterance.onend = () => {
      ...
      window.currentLocalUtterance = null;
    };
    utterance.onerror = () => {
      btnEl.innerHTML = btnEl.id === "speak-btn" ? '<i class="fas fa-volume-up"></i> 読み上げ' : '<i class="fas fa-volume-up"></i> 報告書読み上げ';
      window.currentLocalUtterance = null;
    ...
    window.currentLocalUtterance = utterance;
    window.speechSynthesis.speak(utterance);
  }

  async function speakText(rawText, btnEl) {
    // すでに再生中の音声があれば停止
    ...
    btnEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 音声生成中';
    btnEl.disabled = true;
    ...
    try {
      ...
      if (data.useFallback || !response.ok) {
        // APIキー不在またはエラーの場合はローカル読み上げにフォールバック
        console.warn("GCP TTS error or not configured, falling back to local speech synthesis:", data.error);
        speakTextLocal(text, btnEl);
        return;
      }

      // Base64からオーディオソースを作成して再生
      ...
      audio.onerror = () => {
        console.error("Audio playback error, falling back to local speech synthesis");
        speakTextLocal(text, btnEl);
      };
      ...
      console.warn("Fetch error, falling back to local speech synthesis:", error.message);
      speakTextLocal(text, btnEl);
    }
  }
  ...
  $("speak-btn").addEventListener("click", () => {
    ...
    // 現在、いずれかの音声が再生中であれば停止処理を行う
    if (window.currentAudio || window.speechSynthesis.speaking) {
      if (window.currentAudio) {
        window.currentAudio.pause();
        window.currentAudio = null;
      }
  ...
  $("clear-btn").addEventListener("click", () => {
    ...
    window.speechSynthesis.cancel();
    localStorage.removeItem("nasu_consult_prompt");
    localStorage.removeItem("nasu_consult_answer");
    localStorage.removeItem("nasu_consult_status_class");
    localStorage.removeItem("nasu_consult_status_text");
    $("assistant-answer").textContent = "ここにAIの回答が表示されます。";
    $("status-pill").className = "status-pill idle";
    $("status-pill").textContent = "待機中";
  });
  $("clear-boardroom-btn").addEventListener("click", () => {
    ...
    window.speechSynthesis.cancel();
    localStorage.removeItem("nasu_boardroom_custom_agenda");
    localStorage.removeItem("nasu_boardroom_report");
    localStorage.removeItem("nasu_boardroom_flow_html");
    localStorage.removeItem("nasu_boardroom_status_class");
    localStorage.removeItem("nasu_boardroom_status_text");
  });
  $("simulate-btn").addEventListener("click", () => {
    const result = buildPricingSuggestion();
    $("pricing-output").textContent = result;
    localStorage.setItem("nasu_sim_output", result);
  });
...
function restoreAllStates() {
  // 1. 個別相談の復元
  const savedDept = localStorage.getItem("nasu_consult_selected_dept") || "coo";
  selectDepartment(savedDept);
  
  const savedPrompt = localStorage.getItem("nasu_consult_prompt") || "";
  $("user-prompt").value = savedPrompt;

  const savedAnswer = localStorage.getItem("nasu_consult_answer");
  if (savedAnswer) {
    $("assistant-answer").textContent = savedAnswer;
    $("status-pill").className = localStorage.getItem("nasu_consult_status_class") || "status-pill success";
    $("status-pill").textContent = localStorage.getItem("nasu_consult_status_text") || "回答完了";
    $("speak-btn").style.display = "flex";
    $("consultation-panel").style.display = "block";
  }

  // 2. AI役員会議の復元
  const savedTheme = localStorage.getItem("nasu_boardroom_theme");
  if (savedTheme) {
    $("meeting-theme-select").value = savedTheme;
    syncMeetingDepts(savedTheme);
  }

  const savedCustomAgenda = localStorage.getItem("nasu_boardroom_custom_agenda") || "";
  $("meeting-agenda-custom").value = savedCustomAgenda;

  const savedReport = localStorage.getItem("nasu_boardroom_report");
  if (savedReport) {
    $("boardroom-final-report").textContent = savedReport;
    $("discussion-flow").innerHTML = localStorage.getItem("nasu_boardroom_flow_html") || "";
    $("boardroom-status-pill").className = localStorage.getItem("nasu_boardroom_status_class") || "status-pill success";
    $("boardroom-status-pill").textContent = localStorage.getItem("nasu_boardroom_status_text") || "会議終了・報告書完成";
    $("boardroom-output").style.display = "block";
    $("speak-boardroom-btn").style.display = "flex";
  }

  // 3. 簡易シミュレーターの復元
  const simKeys = ["occ", "adr", "comp", "event", "pace", "lead"];
  simKeys.forEach(k => {
    const val = localStorage.getItem(`nasu_sim_${k}`);
    if (val !== null) {
      const el = $(`${k}-input`);
      if (el) el.value = val;
    }
  });
  const savedSimOutput = localStorage.getItem("nasu_sim_output");
  if (savedSimOutput) {
    $("pricing-output").textContent = savedSimOutput;
  }

  // 4. 入力自動保存イベントのバインド
  $("user-prompt").addEventListener("input", (e) => {
    localStorage.setItem("nasu_consult_prompt", e.target.value);
  });
  $("meeting-agenda-custom").addEventListener("input", (e) => {
    localStorage.setItem("nasu_boardroom_custom_agenda", e.target.value);
  });
  simKeys.forEach(k => {
    const el = $(`${k}-input`);
    if (el) {
      el.addEventListener("input", (e) => {
        localStorage.setItem(`nasu_sim_${k}`, e.target.value);
      });
    }
  });
}
...
function initIframeModal() {
  ...
  const closeBtn = $("iframe-modal-close");
  ...
  document.body.addEventListener("click", (e) => {
    ...
    // 外部リンク（httpから始まるもの）であるか判定
    if (href && href.startsWith("http")) {
      // スマホサイズ (768px以下) の場合のみインラインフレームモーダルで開く
      if (window.innerWidth <= 768) {
        e.preventDefault(); // 通常のリンク遷移をブロック
        iframe.src = href;
...
function init() {
  ...
  // 初期状態では統合個別相談パネルを非表示にする（組織図クリックで展開される）
  const consultPanel = $("consultation-panel");
  if (consultPanel) consultPanel.style.display = "none";
  
  // 最後に、以前の全ての状態を復元（保存された部署の選択やパネル表示状態も含む）
  restoreAllStates();
  
  // インラインプレビューモーダルの初期化（スマホ専用）
  initIframeModal();
  
  $("pricing-output").textContent = buildPricingSuggestion();
}

// 組織詳細カードからのダイレクト相談（ワンクリック自動送信）関数
function triggerDirectConsult(deptId, promptText) {
  // 1. 部署を選択
  selectDepartment(deptId);
  ...
  // 5. 相談回答部分へスクロール
  setTimeout(() => {
    const answerBox = $("assistant-answer");
    if (answerBox) {
